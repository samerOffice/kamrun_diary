...
function optionsframework_tabs() {
	...
	foreach ( $options as $value ) {
		...
		if ( isset( $value['type'] ) && $value['type'] == "heading" ) {
			...
			$tab_url = add_query_arg( array( 'page' => $cur_page_id, 'tab' => $tab_id ), admin_url( 'admin.php', 'relative' ) );
			$tab_class = array(
				'nav-tab',
				$tab_id,
			);

			if ( $tab_id === $active_tab || ( empty( $menu ) && empty( $active_tab ) ) ) {
				$tab_class[] = 'nav-tab-active';
			}

			$menu .= '<a id="' . esc_attr( $tab_id ) . '" class="' . implode( ' ', $tab_class ) .'" title="' . esc_attr( $value['name'] ) . '" href="' . esc_url_raw( $tab_url ) . '">' . esc_html( $value['name'] ) . '</a>';
		}
	}
...
function optionsframework_interface( $options, $cur_page_id ) {
	...
	foreach ( $options as $value ) {
		...
		if ( !in_array( $value['type'], $elements_without_wrap ) ) {
			...
			if ( isset( $_REQUEST['mark'] ) && $value['id'] === $_REQUEST['mark'] ) {
				$class .= ' marked';
			}
		...
		// Basic text input
		case 'text':
			$disabled = ( empty( $value['disabled'] ) ? '' : ' disabled' );
				$output .= '<input id="' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" type="text" value="' . esc_attr( $val ) . '"' . $disabled . '/>';
			break;

		// Password input
		case 'password':
			$disabled = ( empty( $value['disabled'] ) ? '' : ' disabled' );
			$output .= '<input id="' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" type="password" value="' . esc_attr( $val ) . '"' . $disabled . '/>';
			break;
		...
		case 'slider':
			...
			$output .= '<input type="text" class="of-slider-value"' . $str . ' name="' . esc_attr($option_name . '[' . $value['id'] . ']') . '" />';
