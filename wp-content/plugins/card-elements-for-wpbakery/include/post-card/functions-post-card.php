...
if (!function_exists('post_card_posted_by')) {
    ...
    function post_card_posted_by() {
        printf(
                ...
                '<span class="byauthor">%1$s<span class="screen-reader-text">%2$s</span><span class="author vcard"><a class="url fn n" href="%3$s">%4$s</a></span></span>', '<i class="fas fa-user" aria-hidden="true"></i>', __('Posted by', CEWB_TEXTDOMAIN), esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_html(get_the_author())
        );
    }
...
if (!function_exists('post_card_2_posted_by')) {
    ...
    function post_card_2_posted_by() {
        printf(
                /* translators: 1: SVG icon. 2: post author, only visible to screen readers. 3: author link. */
                '<span class="byauthor">%1$s<span class="screen-reader-text">%2$s</span><span class="author vcard"><a class="url fn n" href="%3$s">%4$s</a></span></span>', __(get_avatar(get_the_author_meta('ID'), 40)), __('By', CEWB_TEXTDOMAIN), esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_html(get_the_author())
        );
    }
...
if (!function_exists('post_card_posted_categories')) {
    ...
    function post_card_posted_categories() {
        $categories_list = get_the_category_list(__(', ', CEWB_TEXTDOMAIN));
        if ($categories_list) {
            printf(
                    /* translators: 1: SVG icon. 2: posted in label, only visible to screen readers. 3: list of categories. */
                    '<span class="cat-links">%1$s<span class="screen-reader-text">%2$s</span>%3$s</span>', '', __('Posted in', CEWB_TEXTDOMAIN), $categories_list
            ); // WPCS: XSS OK.
        }
    }
...
if (!function_exists('post_card_posted_tag')) {
    ...
    function post_card_posted_tag() {
        ...
        $tags_list = get_the_tag_list('', __(', ', CEWB_TEXTDOMAIN));
        if ($tags_list) {
            printf(
                    /* translators: 1: SVG icon. 2: posted in label, only visible to screen readers. 3: list of tags. */
                    '<div class="post-card-tags"><span class="tags-links">%1$s<span class="screen-reader-text">%2$s </span>%3$s</span> </div>', '<i class="fa fa-tags" aria-hidden="true"></i>', __('Tags:', CEWB_TEXTDOMAIN), $tags_list
            ); // WPCS: XSS OK.
        }
    }
...
if (!function_exists('post_card_comment_count')) {
    ...
    function post_card_comment_count() {
        if (!post_password_required() && ( comments_open() || get_comments_number() )) {
            ...
            comments_popup_link(sprintf(__('Leave a comment<span class="screen-reader-text"> on %s</span>', CEWB_TEXTDOMAIN), get_the_title()));
